...
name: webapp-testing
description: |
  Testing decision framework for web applications. Routes to three approaches:
  Playwright CLI for automated test scripts, browser automation for manual
  UI/UX verification, agent-browser for quick headless interactions. Use when
  testing, debugging, or verifying web apps.
---
...
|  -> Browser automation (see Claude in Chrome note below)
|
...
> **Full documentation:** `.agents/skills/playwright-cli/`
...
## Approach 2: Browser Automation (Claude in Chrome)

> **Note:** This approach is specific to Claude Code. It requires the Claude Code Chrome extension and CLI. Other AI coding agents should use Playwright CLI or agent-browser instead.

**Manual, conversational testing** using a real Chrome browser with your active login session. You describe what to check -- the agent navigates, inspects, and reports back.

```bash
# Enable from Claude Code CLI
claude --chrome
...
**Limitations:** Desktop only. Not scriptable -- cannot run in CI. One session at a time. Claude Code only.
...
> **Full documentation:** `.agents/skills/agent-browser/` (if available)
...
| Path                                             | Description                                                                            |
| ------------------------------------------------ | -------------------------------------------------------------------------------------- |
| `references/claude-in-chrome.md`                 | Claude in Chrome prerequisites, capabilities, and workflow patterns (Claude Code only) |
...
| `examples/claude-in-chrome/testing-workflows.md` | Conversational testing playbooks (Claude Code only)                                    |
