...
name: shape-spec
description: |
  Gather context, research, and structure planning for significant work.
  Runs a 5-phase, 14-step workflow: scope and research, interactive shaping,
  spec drafting, triple parallel review, and finalization. Outputs a complete
  spec folder under agent-os/specs/. Must be run in plan mode. Does not
...
- **Always ask the user** when gathering information or confirming decisions
...
Launch a subagent for implementation research. This agent uses:
...
Launch a subagent for web research. This agent uses:

- **Web search** for broad discovery across blog posts, discussions, and documentation
- **Web fetching** for extracting content from specific URLs
...
Run directly (no subagent needed) using search and read tools to find:
...
After all three tracks complete, consolidate findings into a research summary and present it to the user:
...
Based on research findings from Step 3, suggest references the user may not have been aware of. Ask the user:
...
Launch 3 review subagents simultaneously. Each reviewer receives the full draft spec content and evaluates: completeness, clarity, feasibility, missing edge cases, standards alignment, and risk.

**Reviewer 1: Claude**

Launch a subagent with the full draft spec. Instruct it to:
...
Present the consolidated feedback to the user:
...
When the spec has passed review (or reached max iterations), ask the user:
